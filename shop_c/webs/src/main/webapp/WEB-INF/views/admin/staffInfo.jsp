	...
	<div class="form-container">
		...
		<form action="staffinfo.htm" method="POST" class="form">
			...
			<div style="margin-bottom: 30px !important" class="wrap">
				<label for="brandsID">Trạng thái</label> <select class="select"
					id="brandsID" name="status" required>
					<option value="1" ${staff.status == 1 ? 'selected' : ''}>Hoạt
						động</option>
					<option value="0" ${staff.status == 0 ? 'selected' : ''}>Không
						hoạt động</option>
				</select>
			</div>
			...
			<div class="wrap">
				<label for="brandsID">Trạng thái tài khoản</label> <select
					class="select" id="brandsID" name="accountStatus" required>
					<option value="1" ${staff.account.status == 1 ? 'selected' : ''}>Hoạt
						động</option>
